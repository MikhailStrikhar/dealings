...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.ОтчетНаименованиеТекущегоВарианта = "Состояние портфеля" Тогда
		Кнопка = Форма.Элементы.Вставить("КнопкаПолучитьПоследниеЦены", Тип("ДекорацияФормы"), 
			Форма.ПодчиненныеЭлементы.КомандыИПоказатели, Форма.ПодчиненныеЭлементы.КомандыИПоказатели.ПодчиненныеЭлементы.ГруппаПоказатель);
		Кнопка.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Получить последние цены'"),,,, "e1cib/command/ОбщаяКоманда.ПолучитьПоследниеЦены");
	КонецЕсли;
