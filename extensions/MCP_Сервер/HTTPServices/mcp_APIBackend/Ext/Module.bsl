...
Функция ОбработатьJSONRPCЗапрос(Запрос)
	...
	Попытка
		...
		// ++
		ИначеЕсли Метод = "resources/templates/list" Тогда
			Результат = ПолучитьСписокШаблоновРесурсов(Параметры);
		// --
		Иначе
			// Неизвестный метод
...
Функция ПолучитьРесурс(Параметры)
	...
	// ++ Стрихарь
	URIРесурса = РаскодироватьСтроку(URIРесурса, СпособКодированияСтроки.URLВКодировкеURL);
	// --
...
Функция ПолучитьСписокШаблоновРесурсов(Параметры) Экспорт
    // Пока шаблонов ресурсов нет. Continue хочет, чтобы метод существовал.
    Результат = Новый Структура;
    Результат.Вставить("resourceTemplates", Новый Массив);
    // nextCursor опционален, но можно добавить для полноты:
    // Результат.Вставить("nextCursor", "");
    Возврат Результат;
КонецФункции